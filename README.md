...
Sao chép
Chỉnh sửa
npm start        # Chạy development server (localhost:3000)
...
📊 Database Schema
Bảng chính:
Books: title, author, isbn, rfidTag, category

Users: name, email, role, phone, address

Loans: userId, bookId, issueDate, dueDate, status

Cards: cardId, userId, isActive

Attendance: userId, checkInTime, checkOutTime
...
🐛 Troubleshooting
⚠️ Backend không kết nối được database:
bash
Sao chép
Chỉnh sửa
...
⚠️ Frontend không kết nối được API:
bash
Sao chép
Chỉnh sửa
# Kiểm tra backend có chạy trên port 5000
# Kiểm tra CORS settings
⚠️ ESP32 không kết nối MQTT:
bash
Sao chép
Chỉnh sửa
# Kiểm tra Wi-Fi connection
...
Tích hợp blockchain để lưu trữ an toàn
